...
function canProceed(step: number, d: Data) {
  if (step === 0) return d.nombre.trim() !== '' && d.taller.trim() !== ''
  ...
  if (step === 2)
    return (
      ...
      d.email.trim() !== '' &&
      d.emailConfirm.trim() !== '' &&
      d.email === d.emailConfirm
    )
  return false
}

export default function InstaladorForm() {
    ...
    return (
      <div className="bg-white/10 backdrop-blur-md rounded-2xl p-10 text-center text-white">
        ...
        <p className="text-white/60 body-text text-sm">
          Nos ponemos en contacto a la brevedad para contarte cómo trabajar con Urban Film.
        </p>
      </div>
    )
  }
      ...
      <div className="overflow-hidden">
        ...
        >
          ...
          <div className="min-w-full space-y-5 pr-px">
            ...
            <Field label="Confirmá tu email">
              <input
                type="email"
                value={data.emailConfirm}
                onChange={(e) => set('emailConfirm', e.target.value)}
                placeholder="tu@email.com"
                className={`${inputCls} ${emailMismatch ? 'border-red-400/70' : ''}`}
              />
              {emailMismatch && (
                <p className="text-red-400 text-xs mt-1.5 body-text">Los emails no coinciden</p>
              )}
            </Field>
          </div>
        </div>
