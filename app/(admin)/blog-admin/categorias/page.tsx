...
import { crearCategoriaBlog, eliminarCategoriaBlog } from '@/app/actions/blog'
import EliminarCategoriaBtn from '../../productos/categorias/_components/EliminarCategoriaBtn'

export const metadata: Metadata = { title: 'Categorías del Blog' }

export default async function CategoriasBlogPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-8">Categorías del Blog</h1>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white border border-gray-200 p-6">
          <h2 className="font-semibold uppercase tracking-wider text-sm mb-6">Nueva categoría</h2>
          <form action={crearCategoriaBlog} className="space-y-4">
            <div>
              ...
              {categorias.map((cat) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{cat._count.posts} posts</p>
                  </div>
                  <EliminarCategoriaBtn
                    id={cat.id}
                    nombre={cat.nombre}
                    count={cat._count.posts}
                    action={eliminarCategoriaBlog}
                  />
                </li>
              ))}
            </ul>
